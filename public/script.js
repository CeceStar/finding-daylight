...
function error() {
  ...
  counter.innerHTML =
    "We could not find your location! Please refresh the page and allow us to see your location." +
    "<br/>" +
    '<div id="refresh-icon"><i class="fas fa-redo"></i></div>';
...
async function getAPIGeoDataSource(x, y) {
  ...
  await fetch("https://us-central1-finding-daylight.cloudfunctions.net/api/", {
    ...
    .catch((error) => {
      ...
      heading.innerText = "Tripped when looking for sunshine :(";
      counter.innerHTML =
        "Something went wrong when getting information regarding the sunset or sunrise. Please refresh the page so we can try get it for you again!" +
        "<br/>" +
        '<div id="refresh-icon"><i class="fas fa-redo"></i></div>';
...
function activateTwilightMode() {
  document.body.style.background =
    "linear-gradient(var(--dark-gray), var(--linen))";
...
function calculateCountDownSunset() {
  ...
  let calculateSunsetDate = newDate.setHours(
    parseInt(sunsetArr[0]),
    parseInt(sunsetArr[1])
  );
...
function calculateCountDownSunriseBeforeMidnight() {
  let newDate = new Date();
  let calculateSunriseDate = newDate.setHours(
    parseInt(sunriseArr[0]),
    parseInt(sunriseArr[1])
  );
